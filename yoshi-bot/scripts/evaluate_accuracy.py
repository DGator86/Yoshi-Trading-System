...
def main():
    ...
    if analysis.horizons:
        ...
        if best.directional_accuracy >= 0.55:
            print(f"\n[PASS] Yoshi predicts {best.directional_accuracy:.0%} accuracy at {best.horizon_time}")
            return 0
        else:
            print(f"\n[FAIL] Accuracy below 55% - model needs improvement")
            return 1
    else:
        print("\n[FAIL] No valid predictions to evaluate")
        return 1
