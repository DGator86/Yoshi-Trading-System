...
def _safe_merge_no_truth(left: pd.DataFrame, right: pd.DataFrame, on: List[str], how: str = "left") -> pd.DataFrame:
    """
    ...
    for fold in harness.generate_folds(features_df):
        ...
        if fold.fold_idx in selected_hparams_per_fold:
            ...
            if "regimes.confidence_floor" in selected_params:
                ...
                fold_regimes_config["constraints_by_species"]["default"]["confidence_floor"] = conf_floor
        ...
        else:            metrics = evaluate_predictions(preds, test_df, "future_return")
        ...
        if isinstance(test_df, pd.DataFrame) and "future_return" in test_df.columns:
            ...
            if len(key_cols) >= 2:
                ...
                assert not any(c in preds.columns for c in ("future_return_x", "future_return_y")), \
